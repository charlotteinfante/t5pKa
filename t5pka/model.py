...
class T5ForProperty(T5ForConditionalGeneration):
    ...
    def __init__(
        ...
        if head_type or getattr(config, "head_type", None):
    ...
    def _init_weights(self, module):
        if module is self:
            ...
            if hasattr(self, "shared"):
                init.normal_(self.shared.weight, mean=0.0, std=factor * 1.0)
            if isinstance(self.lm_head, nn.Sequential):
                for layer in self.lm_head:
                    if isinstance(layer, nn.Linear):
                        init.normal_(layer.weight, mean=0.0, std=factor * 1.0)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if getattr(self, "model_parallel", False):
            ...
            sequence_output = sequence_output.to(next(self.lm_head.parameters()).device)
        ...
        if labels is not None:
            if self.head_type == "classification":
                ...
                # lm_logits = torch.argmax(lm_logits, axis=-1, keepdim=True)
            else:
    ...
    def resize_num_classes(self, new_num_classes: int) -> nn.Embedding:
        """
        Resizes the output layer based on num_classes given
        Arguments:
            new_num_classes (`int`):
                The updated number of classes.
        Return:
            `torch.nn.Sequential`: Pointer to the lm_head of the model.
        """
        lm_head_layers: List[nn.Module] = []
        unit_layer: List[nn.Module] = [
