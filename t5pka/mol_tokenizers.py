...
try:
    from .compat import SimpleVocab, coerce_vocab, load_torch, save_vocab
except ImportError:
    from compat import SimpleVocab, coerce_vocab, load_torch, save_vocab
...
TASK_PREFIX: List[str] = ['Yield:', 'Product:', 'Fill-Mask:', 'Classification:', 'Reagents:', 'Reactants:', 'Pairs:','Prot:','Deprot:'] 
...
class MolTokenizer(ABC, PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.vocab:
            ...
            if hasattr(self, "unique_no_split_tokens"):
    ...
    def merge_vocabs(
        ...
        Args:
            vocabs: vocabularies to be merged
        ...
        return SimpleVocab(
            ...
            max_size=vocab_size if vocab_size else None,
        )
    ...
    def create_vocab(
        ...
        if vocab_file:
            if not os.path.isfile(vocab_file):
                ...
                self.vocab: SimpleVocab = self.merge_vocabs(
                    [coerce_vocab(load_torch(vocab_file))],
            ...
            for i, source_file in enumerate(source_files):
                ...
                vocabs[i] = SimpleVocab(counter[i], specials=specials)
            self.vocab = self.merge_vocabs([vocabs[i] for i in range(len(source_files))], vocab_size=vocab_size)
        else:
            self.vocab = None

    def get_vocab(self) -> Dict[str, int]:
        vocab_obj = self.__dict__.get("vocab")
        if isinstance(vocab_obj, SimpleVocab):
            vocab = {token: i for i, token in enumerate(vocab_obj.itos)}
        else:
            vocab = {}
    ...
    def _convert_token_to_id(self, token: str) -> int:
        ...
        assert isinstance(self.vocab, SimpleVocab),\
            'No vocabulary found! Need to be generated at initialization or using .create_vocab method.'
        return self.vocab.stoi.get(token, self.vocab.stoi[self.unk_token])
